  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setFile(null);
        setFileError("");
        toast.success("قطعه با موفقیت به‌روزرسانی شد");
        window.dispatchEvent(new CustomEvent("partsUpdated"));
      } else {
        toast.error(data.message || "خطا در به‌روزرسانی قطعه");
      }
    } catch (error) {
      console.error("Error updating part:", error);
      toast.error("خطا در به‌روزرسانی قطعه");
    } finally {
      setLoading(false);
    }
  };
              ...
              {(file || part?.glbPath) && (
                <div className="mt-4">
                  ...
                  >
                    <ModelPreview
                      file={file}
                      url={!file && part?.glbPath ? part.glbPath : null}
